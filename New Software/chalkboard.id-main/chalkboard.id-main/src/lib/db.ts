...
function getPgliteDataDir(): string {
  // If we're deployed on Netlify or Vercel edge/node but forcing desktop DB
  if (process.env.NETLIFY === 'true' || process.env.VERCEL === '1') {
    return '/tmp/jrsnooker-db';
  }
...
function createConnectionString(): string | null {
  ...
  const database = config.database.database || 'jrsnooker_db';
