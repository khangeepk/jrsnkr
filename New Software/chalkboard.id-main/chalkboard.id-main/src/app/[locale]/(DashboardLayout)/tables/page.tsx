...
const TablesManagement = () => {
  ...
  const [sessionData, setSessionData] = useState({
    players: ['', '', '', '', '', ''],
    gameType: 'single' as 'single' | 'double' | 'century_individual' | 'century_team' | 'century_group',
    nominatedPayerName: '',
    plannedDuration: 60,
    pricingPackageId: ''
  });
  ...
  const validatePlayers = () => {
    const filledPlayers = sessionData.players.filter(p => p.trim() !== '');
  ...
  const processFnbOrder = async () => {
    ...
    try {
      ...
      const orderPayload = {
        ...
        customerName: session.players ? JSON.parse(session.players as any)[0]?.name || 'Unknown' : 'Unknown',
        customerPhone: null,
  ...
  const handleStartSession = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setSelectedTable(null);
        setSessionData({ players: ['', '', '', '', '', ''], gameType: 'single', nominatedPayerName: '', plannedDuration: 60, pricingPackageId: '' });
        fetchTables();
      } else {
        const error = await response.json();
  ...
  const handleEndSession = async (tableId: number) => {
    try {
      // 1. Fetch current session data to identify payer
      const sessionResponse = await fetch(`/api/tables/${tableId}/current-session`);
      let payerName = 'Unknown';

      if (sessionResponse.ok) {
        const sessionData = await sessionResponse.json();
        const players = sessionData.session?.players ? JSON.parse(sessionData.session.players) : [];
        payerName = sessionData.session?.nominatedPayerName || (players[0]?.name || 'Unknown');
      }

      // 2. Fetch outstanding credit for this logical payer
      let totalOutstanding = 0;
      try {
        const creditRes = await fetch(`/api/ledger/credit?playerName=${payerName}`);
        if (creditRes.ok) {
          const creditData = await creditRes.json();
          // Assuming API returns array of outstanding credits, matching Phase 5 implementation
          totalOutstanding = creditData.entries?.reduce((sum: number, entry: any) => sum + parseFloat(entry.outstandingBalance), 0) || 0;
        }
      } catch (e) {
        console.error("Failed to fetch outstanding credit", e);
      }
  ...
  return (
    <div className="space-y-6">
              ...
              <div className="p-4">
                ...
                <div className="flex justify-between items-start mb-4">
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-bodytext mt-1 truncate">
                      {session?.players ? JSON.parse(session.players as any).map((p: any) => p.name).join(', ') : 'Unknown'}
                    </p>
                  </div>
                  <Badge
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-bodytext truncate">
                Players: {selectedTable && sessions[selectedTable.id]?.players ? JSON.parse(sessions[selectedTable.id].players as any).map((p: any) => p.name).join(', ') : 'N/A'}
              </p>
            </div>
          </div>
        </Modal.Header>
                  ...
                  <div className="space-y-2 max-h-96 overflow-y-auto">
                    {filteredItems.map(item => {
                      ...
                      return (
                        <div
                          key={item.id}
                          className={`flex items-center justify-between p-3 rounded-lg border-2 transition-all ${isOutOfStock ? 'opacity-50 cursor-not-allowed' : 'hover:shadow-md cursor-pointer'
                            }`}
                          style={{
              ...
              {sessionData.players.map((player, index) => (
                <TextInput
                  ...
                  onChange={(e) => {
                    ...
                    setSessionData({ ...sessionData, players: newPlayers });
                  }}
                  placeholder={`Player ${index + 1} Name ${index < 2 ? '*' : ''}`}
                  required={index < 2}
                />
              ))}
            </div>
            <div>
              <Label htmlFor="gameType" value="Game Type" />
              <Select
                id="gameType"
                value={sessionData.gameType}
                onChange={(e) => setSessionData({ ...sessionData, gameType: e.target.value as any })}
              >
                <option value="single">Single Game</option>
                <option value="double">Double Game</option>
                <optgroup label="Century Game">
                  <option value="century_individual">Individual</option>
                  <option value="century_team">Team</option>
                  <option value="century_group">Group</option>
                </optgroup>
              </Select>
            </div>
            {sessionData.gameType === 'century_group' && (
              <div>
                <Label htmlFor="nominatedPayer" value="Nominated Payer *" />
                <Select
                  id="nominatedPayer"
                  value={sessionData.nominatedPayerName}
                  onChange={(e) => setSessionData({ ...sessionData, nominatedPayerName: e.target.value })}
                  required
                >
                  <option value="">Select Payer</option>
                  {sessionData.players.filter(p => p.trim() !== '').map((p, i) => (
      ...
      <MoveSessionModal
        ...
        customerName={selectedSession?.players ? JSON.parse(selectedSession.players as any).map((p: any) => p.name).join(', ') : ''}
